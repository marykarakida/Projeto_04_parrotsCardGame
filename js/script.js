...
function inserirCartas(cartas) {
    ...
    start = Date.now();
    intervalo = setInterval(medirTempo,1000,start);
}
...
function medirTempo() {
    duration = parseInt((Date.now() - start) / 1000);
    document.querySelector(".tempo").innerHTML = `Timer: ${duration}s`;
}
...
function finalizarJogo() {
    alert(`Você ganhou em ${jogadas} jogadas e ${parseInt(duration)} segundos!`);
